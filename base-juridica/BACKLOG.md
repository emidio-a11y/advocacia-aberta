...
| `BASE-030` | O monitor não alcança 4 fontes do STF pelo runner do GitHub Actions: `sumulas_stf`, `sumulas_vinculantes`, `temas_rg_stf` e `informativo_stf` falham com erro SSL de certificado (curl 60). Causa exata não confirmável remotamente (o STF responde 403 a IP fora do país) | Com o `BASE-029`, a próxima execução agendada mostra o motivo textual do curl (cadeia incompleta vs. CA desatualizada vs. transitório); classificar e, se for cadeia incompleta, fornecer os intermediários ou um bundle de CA no workflow | aberto |

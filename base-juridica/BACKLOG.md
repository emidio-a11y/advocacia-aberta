...
| `BASE-031` | Súmulas do STJ e Jurisprudência em Teses ficavam sem verificação: o monitor agendado recebia 403 do SCON. A hipótese inicial — WAF bloqueando cliente que não fosse navegador — **estava errada** | O bloqueio é **por origem de rede, não por cliente**: o mesmo `curl`, com o mesmo User-Agent do monitor, recebe 403 de IP de datacenter e **200** de uma rede aceita, devolvendo catálogo íntegro (676 súmulas, batendo com o snapshot; JT na edição 284). Navegador headless é desnecessário. O monitor da nuvem passa a pular essas famílias por `--excluir` (com o relatório **declarando** a exclusão, sem omissão silenciosa) e elas são verificadas por `ferramentas/manutencao/monitorar-stj-local.sh`, agendado numa rede aceita. CKAN confirmado sem dataset dessas famílias (dos 21, só `precedentes-qualificados`) | **concluído em 2026-07-22** |

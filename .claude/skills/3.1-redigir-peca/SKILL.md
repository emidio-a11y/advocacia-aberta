...
Se o texto exato de uma lei, precedente ou depoimento não estiver na fonte verificada, **não use** — escreva `[verificar na fonte]` e siga em frente. Toda lei ou precedente deve vir de `/buscar-fontes` ou `/buscar-tjpr` já registrado na fundamentação. A única escrita livre é a análise e a narrativa ao redor das citações.

## Instrução

`$ARGUMENTS` traz o caminho da pasta do caso e, opcionalmente, o tipo de peça. Exemplos:
...
Os caminhos são relativos à raiz do projeto (a pasta onde você abriu o Claude Code).
...
- Se o tipo veio em `$ARGUMENTS`, use-o.
...
- Sem SUMARIO.md → rode `/organizar-caso` primeiro.
- Sem DIAGNOSTICO.md → rode `/diagnosticar` primeiro.
- Sem fundamentação → rode `/buscar-fontes` e `/buscar-tjpr` primeiro.

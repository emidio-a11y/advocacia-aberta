...
A skill lê o `SUMARIO.md` do caso (produzido por `/organizar-caso`) e os documentos relevantes, e produz o `DIAGNOSTICO.md`.

## Instrução

`$ARGUMENTS` deve ser o caminho relativo da pasta do caso (ex.: `casos/contrato-prestacao-servicos` ou `casos/0001234-56`). Se vier vazio, peça ao usuário o caminho da pasta do caso.

Os caminhos abaixo são todos relativos à raiz do projeto (a pasta onde você abriu o Claude Code).

Se o `SUMARIO.md` ainda não existir, rode `/organizar-caso $ARGUMENTS` primeiro.

---

### Passo 1 — Ler o SUMARIO.md

Leia `$ARGUMENTS/analise/SUMARIO.md`.
...
Se existir `analise/PROVA_ORAL.md` (produzido por `/transcrever` + análise da audiência), leia-o primeiro — ele já consolida a síntese por depoente.
...
Escreva `$ARGUMENTS/analise/DIAGNOSTICO.md` (crie a pasta `analise/` se não existir):

...
description: Instala automaticamente as ferramentas que algumas skills precisam (transcrição de áudio, busca de jurisprudência); use quando uma skill avisar que falta uma ferramenta.
---

# Preparar ambiente

A maioria das skills deste kit é texto puro e funciona sem instalar nada. Apenas três
...
```bash
which bun uv whisper-cli ffmpeg
```
...
- **macOS:** tudo funciona de forma suave. Usa o Homebrew para instalar o whisper. Se o
  usuário não tiver o Homebrew, oriente a instalar primeiro seguindo as instruções de
  https://brew.sh.
- **Linux:** `bun` e `uv` funcionam normalmente. Para a transcrição, o usuário precisa
  instalar o whisper.cpp e o ffmpeg pelo gerenciador de pacotes da própria distribuição
  (por exemplo, `apt`).
- **Windows:** `bun` e `uv` pedem o WSL (Subsistema do Windows para Linux). A transcrição
  com whisper é mais complicada no Windows — nesse caso, sugira ao usuário usar um serviço
  de transcrição online (por exemplo, o Fireflies, mencionado na palestra) como alternativa.
...
Depois do setup concluído, é só voltar a usar **/transcrever**, **/buscar-fontes** ou
**/buscar-tjpr** normalmente.

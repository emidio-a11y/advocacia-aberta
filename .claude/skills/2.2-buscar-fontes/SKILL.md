...
Use `$ARGUMENTS` como query de pesquisa. Se vazio, peça ao usuário o tema.

### Passo 1 — Executar busca ampla

```bash
bun run ferramentas/pesquisa/busca_delfus/src/cli.ts buscar "$ARGUMENTS" 5
```

### Passo 1b — Se necessário, busca focada em legislação

```bash
bun run ferramentas/pesquisa/busca_delfus/src/cli.ts legislacao "$ARGUMENTS" todos 3
```
...
Se nenhum resultado for relevante, sugira variações de query ou proponha busca em tribunal específico com `/buscar-tjpr`.

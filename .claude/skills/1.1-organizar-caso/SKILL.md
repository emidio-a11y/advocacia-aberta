...
Esta skill é uma instrução para você (Claude) organizar a pilha de documentos de um caso. "Caso" aqui é qualquer conjunto de material jurídico: os autos de um processo judicial, mas também os documentos de um contrato, as peças de uma due diligence, um dossiê de investigação ou os anexos que um cliente enviou.
...
`$ARGUMENTS` deve ser o caminho relativo da pasta do caso, a partir da raiz do projeto (ex.: `casos/meu-caso`). Se vier vazio, peça ao usuário o caminho da pasta do caso.

A raiz do projeto é a pasta onde você abriu o Claude Code. Todos os caminhos abaixo são relativos a ela — nunca use caminhos absolutos.

Os documentos do caso ficam, por convenção, em `$ARGUMENTS/autos/`. Se a pasta não existir ou os arquivos estiverem soltos direto em `$ARGUMENTS/`, trabalhe com o que encontrar e siga em frente.
...
```bash
ls -1 "$ARGUMENTS"/autos/ 2>/dev/null || ls -1 "$ARGUMENTS"/
```

Anote os arquivos que dá para ler como texto: PDFs, .docx, .txt, .md. Áudio e vídeo (.mp4, .webm, .m4a, .wav) ficam de fora desta skill — para esses, use `/transcrever` primeiro.
...
```bash
python3 ferramentas/processamento/split-autos/split.py "$ARGUMENTS/autos/integra.pdf"
```
...
Para cada documento, use o `Read` para extrair o texto. Em documentos longos, leia as primeiras páginas para entender do que se trata; em documentos curtos, leia tudo. Para cada um, levante:
...
Crie a pasta `analise/` se não existir e escreva `$ARGUMENTS/analise/SUMARIO.md` com esta estrutura (adapte os rótulos ao tipo de caso — os exemplos entre colchetes são só orientação):
...
- Que o `SUMARIO.md` agora é o ponto de partida do caso: use `/diagnosticar` para mapear as fragilidades e `/redigir-peca` para produzir a peça.

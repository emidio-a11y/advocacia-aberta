...
`$ARGUMENTS` deve ser o caminho do arquivo de mídia (áudio ou vídeo), relativo à
raiz do projeto. Exemplos:
...
Se `$ARGUMENTS` estiver vazio, detecte o arquivo aberto no editor ou pergunte ao
usuário qual é o caminho.

Os caminhos abaixo são todos relativos à raiz do projeto (a pasta onde você abriu
o Claude Code).
...
**oriente o usuário a rodar `/preparar-ambiente`**, que instala tudo
automaticamente (as ferramentas e o modelo de voz). Não tente instalar nada aqui;
apenas aponte para `/preparar-ambiente` e pare.
...
```bash
python3 ferramentas/processamento/transcrever/transcrever.py "$ARGUMENTS"
```
...
- Próximos passos sugeridos, conforme o caso: rotular os `[?]` (se ainda não
  feito), ou `/diagnosticar` para revisar a tese à luz do que foi dito, ou
  `/redigir-peca` para usar os trechos como prova.

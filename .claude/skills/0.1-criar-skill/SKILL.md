...
Se `$ARGUMENTS` trouxer uma ideia de tarefa (ex.: "resumir contratos de locação"),
use como ponto de partida. Se vier vazio, comece perguntando pela tarefa.
...
- "Tem algum dado que **precisa estar organizado antes**?" Se for áudio, aponte
  `/transcrever`; se for uma pilha de documentos, aponte `/organizar-caso`.
...
Crie o arquivo em `.claude/skills/<nome-escolhido>/SKILL.md` com esta estrutura:
...
Use `$ARGUMENTS` para <o que o usuário informa ao chamar, ex.: o caminho da pasta do caso>.
...
- Se a tarefa se conecta a outras skills do kit, referencie-as pelo comando
  (ex.: "antes, rode `/organizar-caso`"; "depois, use `/revisar-peca`").

### Passo 7 — Explicar como usar

Ao terminar, diga ao usuário:
- "Pronto: criei a skill **<nome>**. Para usar, é só digitar `/<nome>` aqui no Claude."
- Sugira testá-la agora com um exemplo real.
- Lembre que ela pode ser ajustada quando quiser — é só abrir o arquivo
  `.claude/skills/<nome>/SKILL.md` e editar, ou chamar `/criar-skill` de novo para
  refinar.

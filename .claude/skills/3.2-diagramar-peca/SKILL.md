...
`$ARGUMENTS` é o caminho do arquivo da peça (ex.: `casos/meu-caso/pecas/contestacao-v1.md`).
Se vier vazio, pergunte ao usuário qual peça diagramar.
...
- Preencha `#fundamento(...)` e `#citacao(...)` só com o que está no texto. Se a peça
  ...
  invente. Se quiser enriquecer com a fonte real, use antes `/buscar-fontes` ou
  `/buscar-tjpr` e só então preencha.
...
Antes de diagramar, vale rodar `/revisar-peca` — diagramar uma peça com erro só deixa
o erro mais bonito. A ordem natural é: `/redigir-peca` → `/revisar-peca` → `/diagramar-peca`.
